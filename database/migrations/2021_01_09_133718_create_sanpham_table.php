...
class CreateSanphamTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('sanpham', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->bigIncrements('id')->comment('Id sản phẩm');
            ...
            $table->string('sp_mau', 200)->comment('Màu của sản phẩm');
            ...
            $table->unsignedSmallInteger('nsx_ma')->comment('Mã nhà sản xuất');
            
            $table->unique(['sp_ten']);
            $table->foreign('nsx_ma') //cột khóa ngoại là cột `l_ma` trong table `sanpham`
                ->references('nsx_ma')->on('nhasanxuat') //cột sẽ tham chiếu đến là cột `l_ma` trong table `loai`
                ->onDelete('CASCADE')
                ->onUpdate('CASCADE');
        });
    }
